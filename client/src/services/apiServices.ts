...
export const getUsersList = async(page: number): Promise<{users: IUser[]; totalPages: number;totalCount: number}> => {
    try {
        const response = await axios.get(`http://localhost:5000/user?page=${page}&limit=12`)
